...
export default class SaveButton extends React.PureComponent {
  ...
  _save = () => {
    ...
    task_data_map.update("description", value =>
      this.props.task_description.trim()
    );

    let old_category = Map(this.props.old_task_data_map).get("category"),
      new_category = task_data_map.get("category"),
      old_priority_value = Map(this.props.old_task_data_map).getIn([
        "priority",
        "value"
      ]),
            ...
            return List(tasks).delete(
              List(tasks).findIndex(
                (task_data, index, list) =>
                  task_data_map.get("id") === Map(task_data).get("id")
              )
            );
          }
        },
  ...
  render() {
    return (
      <TouchableOpacity style={styles.container} onPress={this._save}>
