...
export default class YearChartHolder extends React.PureComponent {
    ...
    _updateYearChartData = () => {
        ...
        if (year_chart_stats_map.hasIn([year_toString, "current"])) {
            let current = List(year_chart_stats_map.getIn([year_toString, "current"]))

            this.y_max = current.reduce((total, value) => total + value)
        }

        if (year_chart_stats_map.hasIn([year_toString, "completed_priority_array"])) {
            let completed_priority_array = List(year_chart_stats_map.getIn([year_toString, "completed_priority_array"]))
    ...
    shouldComponentUpdate(nextProps, nextState) {
        ...
        return this.state !== nextState
            || Map(this.props.year_chart_stats).get(year_toString) !== Map(nextProps.year_chart_stats).get(year_toString)
    }
