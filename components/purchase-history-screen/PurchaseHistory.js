    ...
    return (
      ...
      >
        <FlatList
          ...
          data={OrderedMap(this.props.purchase_history)
            .toArray()
            .reverse()}
        />
      </View>
...
class PurchasedTimestamp extends React.PureComponent {
  ...
  _keyExtractor = (item, index) =>
    `purchased-detailed-timestamp-${this.props.timestamp_toString}-id-${item[0]}`;
  ...
  render() {
    let timestamp = parseInt(this.props.timestamp_toString),
      ...
      displaying_date = `${
        this.day_in_week_text_array[date.getDay()]
      } ${date.getDate()} ${
        this.month_text_array[date.getMonth()]
      } ${date.getFullYear()}`;
    ...
    return (
      ...
      >
        <TouchableOpacity onPress={this._ToggleDisplayingOptionsBool}>
          <Text style={styles.timestamp_text}>{displaying_date}</Text>
        </TouchableOpacity>
        ...
        <Collapsible collapsed={this.state.should_display_options_collasped}>
          ...
          >
            ...
            >
              <FontAwesomeIcon icon={faTrashAlt} color="#EB5757" size={16} />

              <Text style={styles.delete_all_records_text}>
                Delete all records
              </Text>
            </TouchableOpacity>
          </View>
...
class PurchasedDetailedTimestamp extends React.PureComponent {
  _keyExtractor = (item, index) =>
    `purchase-item-detailed-timestamp-${this.props.detail_timestamp}-id-${item[0]}`;

  _renderItem = ({ item, index }) => {
    return <PurchasedItem data={item[1]} />;
  };
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          <Text style={styles.item_title}>{item_title}</Text>

          <Text style={styles.item_latest_timestamp}>
        ...
        >
          <Text style={styles.item_value}>{`${item_value} pts`}</Text>
